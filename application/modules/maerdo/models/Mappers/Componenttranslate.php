...
class Maerdo_Model_Mappers_Componenttranslate extends Maerdo_Model_Mappers_Abstract {

    public function getDbTable()  {
       if (null === $this->_dbTable) {
            $this->setDbTable('Maerdo_Model_Dbtable_Componenttranslate');
       }
    ...
    public function fetchAll()  {
        ...
        foreach ($resultSet as $row) {
            $entry = new Maerdo_Model_Componenttranslate();
            $entry->setId($row->id)
     ...
     * saves current row
     *
     * @param Maerdo_Model_Component__translate $model
     *
     */
     
    public function save(Maerdo_Model_Componenttranslate $model,$ignoreEmptyValuesOnUpdate=true) {
     ...
     *
     * @param new Maerdo_Model_Component__translate $model
     * @return array
     *
     */
     ...
     * @param string $field
     * @param mixed $value
     * @param Maerdo_Model_Componenttranslate $model
     * @return array
     */
    public function findByField($field, $value, $model)  {            
            ...
            foreach ($rows as $row) {
                    $model=new Maerdo_Model_Componenttranslate();
                    $result[]=$model;
                    $model->setId($row->id)
						  ->setModule_id($row->module_id)
						  ->setIdentifiant($row->identifiant)

...
class Front_Model_Mappers_Componentauth extends Front_Model_Mappers_Abstract {

    public function getDbTable()  {
       if (null === $this->_dbTable) {
            $this->setDbTable('Front_Model_Dbtable_Componentauth');
       }
    ...
    public function fetchAll()  {
        ...
        foreach ($resultSet as $row) {
            $entry = new Front_Model_Componentauth();
            $entry->setId($row->id)
     ...
     * saves current row
     *
     * @param Front_Model_Component__auth $model
     *
     */
     
    public function save(Front_Model_Componentauth $model,$ignoreEmptyValuesOnUpdate=true) {
     ...
     *
     * @param new Front_Model_Component__auth $model
     * @return array
     *
     */
     ...
     * @param string $field
     * @param mixed $value
     * @param Front_Model_Componentauth $model
     * @return array
     */
    public function findByField($field, $value, $model)  {            
            ...
            foreach ($rows as $row) {
                    $model=new Front_Model_Componentauth();
                    $result[]=$model;
                    $model->setId($row->id)
						  ->setModule_id($row->module_id)
						  ->setTable($row->table)

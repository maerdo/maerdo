...
class Maerdo_Model_Mappers_Componentcachebackendfileoption extends Maerdo_Model_Mappers_Abstract {

    public function getDbTable()  {
       if (null === $this->_dbTable) {
            $this->setDbTable('Maerdo_Model_Dbtable_Componentcachebackendfileoption');
       }
    ...
    public function fetchAll()  {
        ...
        foreach ($resultSet as $row) {
            $entry = new Maerdo_Model_Componentcachebackendfileoption();
            $entry->setId($row->id)
                  ->setCc_id($row->cc_id)
     ...
     * saves current row
     *
     * @param Maerdo_Model_Component__cache__backend__file__option $model
     *
     */
     
    public function save(Maerdo_Model_Componentcachebackendfileoption $model,$ignoreEmptyValuesOnUpdate=true) {
     ...
     *
     * @param new Maerdo_Model_Component__cache__backend__file__option $model
     * @return array
     *
     */
     ...
     * @param string $field
     * @param mixed $value
     * @param Maerdo_Model_Componentcachebackendfileoption $model
     * @return array
     */
    public function findByField($field, $value, $model)  {            
            ...
            foreach ($rows as $row) {
                    $model=new Maerdo_Model_Componentcachebackendfileoption();
                    $result[]=$model;
                    $model->setId($row->id)
						  ->setCc_id($row->cc_id)
						  ->setCache_dir($row->cache_dir)
